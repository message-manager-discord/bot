...
class MessagesCog(commands.Cog):
    ...
    async def check_channel(self, ctx : commands.Context, channel):
        ...
        if channel == None:
            ...
            try:
                ...
                pass
            get_channel.content
            channel = await commands.TextChannelConverter().convert(ctx, get_channel.content)
    ...
    async def check_message_id(self, ctx : commands.Context, channel, message):
        ...
        if message == None:
            ...
            try:
                ...
                raise self.bot.errors.InputContentIncorrect("I can't find that message!")
    ...
    async def send_message_info_embed(self, ctx : commands.Context, command_type, author, content, message):
        title = 'Sent'
        list_content = [
    ...
    async def send(self, ctx : commands.Context, channel : discord.TextChannel = None, *, content=None):
        try:
    ...
    async def edit(self, ctx : commands.Context, channel : discord.TextChannel = None, message_id=None, *, content=None):
        try:
            await ctx.message.delete()
        except discord.errors.Forbidden:
            pass
    ...
    async def delete(self, ctx : commands.Context, channel : discord.TextChannel = None, message_id=None):
        def is_correct(m):
            return m.author == ctx.author
        try:
            await ctx.message.delete()
        ...
        embed.add_field(name='Content', value = msg.content or msg.embeds[0].title, inline = False)
        ...
        if choice.content.lower() == 'yes':
            ...
            await self.send_message_info_embed(
                ...
                'delete',
                ctx.author,
                msg.content or msg.embeds[0].title,
                msg
            )
        else:
            embed = discord.Embed(
            ...
            embed.add_field(title='', value ='{ctx.author.mention}chose not to delete the message')
            await ctx.send(embed=embed)

    @commands.command(name="fetch")
    async def fetch(self, ctx : commands.Context, channel : discord.TextChannel = None, message_id=None):
        try:
            await ctx.message.delete()
        except discord.errors.Forbidden:
            pass
